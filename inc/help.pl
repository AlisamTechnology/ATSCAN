...
## HELP
  ...
  ."  -m             | Set engine motors default bing \n"
  ."                 | EX: -m [Bing: 1][Google: 2][Ask: 3][Yandex: 4][Sogou: 5][All: all]\n"
  ."  --proxy-random | Random proxy \n"
  ."                 | [EX: --proxy-random list.txt] or --proxy-random \"socks://localhost:9050\"]\n"
  ...
  ."                 | use with -v --status --ifinurl --s/regex commands\n"
  ."  --notin        | Get targets where string does not exist in html\n"
  ...
  ."  --noquery      | Remove string value from Query url \n"
  ."                 | Ex: site.com/index.php?id=string] \n"  
  ...
  ."  [OTHER]        | To separate all others values (dork exploit payload proxy target..) \n"
  ."                 | Ex: --dork \"dork1 [OTHER]DORK2 [OTHER]DORK3\"\n"
