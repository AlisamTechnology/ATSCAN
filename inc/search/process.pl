...
our ($browserLang, $mrand, $motorparam, $motor, $motor1, $motor2, $motor3, $motor4, $motor5, $mrandom, $googleDomain, $prandom, $proxy, $psx, $mlevel, $ifinurl, $unique, $mdom, 
     $searchRegex, $Target, $dork, $ua, $Id, $MsId, $V_SEARCH,$nolisting, $msites);
...
######################################################################################################################################################################################################
######################################################################################################################################################################################################
our ($limit, $noinfo, $brandom, $agent, $post, $get, $timeout, $replace, $outdir, $output, $data, $noQuery, $V_IP, $with, $eMails, $searchIps, @OTHERS);
######################################################################################################################################################################################################
######################################################################################################################################################################################################
## INFO URL SCAN
sub printInfoUrl {
...
sub browseUrl {
  ...
  if ($o<$limit) {
    if (!defined $noinfo) { 
      ...
      if (defined $output) { print $c[1]."    OUTPUT  ". $c[10]."$outdir/$output\n"; }
    }
  }
